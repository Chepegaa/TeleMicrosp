...
//      }
    ...
    var redirectedUrl = "redirected url";
    if (resp.keywordRedirect && resp.keywordRedirect[redirectedUrl]) {
        if(resp.keywordRedirect[redirectedUrl].indexOf('index.html') === 0){ // Relative URL
    ...
    for (var i = 0; i < rawData.category.length; i++) {
      ...
      if (!skipCategory) {
        categoryDimension.options.push({
          ...
          level: currentCategory.crumb.replace(/^\/|\/$/g,'').split('index.html').length, // the top level category is level 1
